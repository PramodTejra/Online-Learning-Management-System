...
if(isset($_POST['submit'])){
    ...
    if(mysqli_num_rows($check_result) > 0){
        ...
        if(mysqli_num_rows($subcode_check_result) > 0){
            ...
            $insert_sql = "INSERT INTO std_result (subname, subcode, schno, mini, mid, assignment, end_term, grade) VALUES ('$subname', '$subcode','$schno', '$mini', '$mid', '$assignment', '$end_term','$grade')";
            $result = mysqli_query($data, $insert_sql);
        } else {
            // Update the existing record if schno and subcode are the same
            $update_sql = "UPDATE std_result SET mini='$mini', mid='$mid', assignment='$assignment', end_term='$end_term', grade='$grade' WHERE schno='$schno' AND subcode='$subcode'";
    ...
    if($result){
        ...
        header("Refresh: 0.2; URL=teacher_course.php");
